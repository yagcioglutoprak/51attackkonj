...
*Setup instructions for a Gitian build of Konjungate Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Konjungate
...
- [Building Konjungate Core](#building-konjungate-core)
...
Clone the git repositories for Konjungate Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/konjungatecoin/konjungate
```
...
in the Konjungate Core repository under 'Fetch and build inputs' to install sources which require
...
To build Konjungate Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Konjungate Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/konjungate/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/konjungatecoin/konjungate
...
For example:
```bash
URL=https://github.com/crowning-/konjungate.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit konjungate=${COMMIT} --url konjungate=${URL} ../konjungate/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit konjungate=${COMMIT} --url konjungate=${URL} ../konjungate/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit konjungate=${COMMIT} --url konjungate=${URL} ../konjungate/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the konjungate git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../konjungate/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/konjungate.git
SIGPATH=/some/root/path/konjungate-detached-sigs.git

./bin/gbuild --url konjungate=${BTCPATH},signature=${SIGPATH} ../konjungate/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/konjungate-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/konjungate-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/konjungate-osx-build.assert
```
...
[konjungate/gitian.sigs](https://github.com/konjungatecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
