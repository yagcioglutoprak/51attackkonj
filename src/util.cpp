...
const char * const BITCOIN_CONF_FILENAME = "konjungate.conf";
const char * const BITCOIN_PID_FILENAME = "konjungated.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "konjungate" is a composite category enabling all Konjungate-related debug output
            if(ptrCategory->count(string("konjungate"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KonjungateCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KonjungateCore
    // Mac: ~/Library/Application Support/KonjungateCore
    // Unix: ~/.konjungatecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KonjungateCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KonjungateCore";
#else
    // Unix
    return pathRet / ".konjungatecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override konjungate.conf
