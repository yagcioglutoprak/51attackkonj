...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Konjungate address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no konjungate: URI
    if(!uri.isValid() || uri.scheme() != QString("konjungate"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::KONJ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because konjungate:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("konjungate://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "konjungate:");
    }
...
{
    QString ret = QString("konjungate:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KONJ, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open konjungate.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Konjungate (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Konjungate (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("konjungate-%s.lnk", chain);
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Konjungate Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Konjungate Core app to startup item list
