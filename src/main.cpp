...
#if defined(NDEBUG)
#error "Konjungate Core cannot be compiled without assertions."
#endif
...

    {
        ...
        BOOST_FOREACH (CTxMemPool::txiter ancestorIt, setAncestors) {
            ...
            if (setConflicts.count(hashAncestor)) {
                return state.DoS(10, error("AcceptToMemoryPool: %s spends conflicting transaction %s", hash.ToString(), hashAncestor.ToString()),
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++) {
                ...
                if (!setConflictsParents.count(tx.vin[j].prevout.hash)) {
                    ...
                    if (pool.mapTx.find(tx.vin[j].prevout.hash) != pool.mapTx.end())
                        return state.DoS(0, error("AcceptToMemoryPool: replacement %s adds unconfirmed input, idx %d", hash.ToString(), j),
...
{
    RenameThread("konjungate-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // KONJ : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(KONJ): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(KONJ): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, int prevBlockHeight, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // KONJ : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH (const CTxIn& txin, tx.vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(KONJ): transaction %s conflicts with transaction lock %s", tx.GetHash().ToString(), hashLocked.ToString()),
                        REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    } else {
        LogPrintf("CheckBlock(KONJ): spork is off, skipping transaction locking checks\n");
    }
    ...
    BOOST_FOREACH (const CTransaction& tx, block.vtx) {
        ...
        if (sporkManager.IsSporkActive(SPORK_15_CHARITY_PAYMENT_ENFORCEMENT) && (prevBlockHeight + 1 > Params().GetConsensus().nCharityPaymentsStartBlock)) {
    ...
    if (!charityTransaction) {
        ...
        return state.DoS(0, error("CheckBlock(KONJ): transaction %s does not contains charity transaction", block.txoutCharity.GetHash().ToString()), REJECT_INVALID, "charity-not-found");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Konjungate Related Inventory Messages
